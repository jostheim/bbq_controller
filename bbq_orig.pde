...
#define KEEPALIVE_ADDRESS 192, 168, 1, 20
#define BLOWER_PIN 9
...
// WPA/WPA2 passphrase
const prog_char ssid[] PROGMEM = { "jamie1976" }; // max 32 bytes
...
unsigned int food_temp_0 = 0;
unsigned int food_temp_1 = 0;
unsigned int food_temp_2 = 0;
unsigned int food_temp_3 = 0;
unsigned int seconds_on_current = 0;
char current = 'P';
...
//		WiServer.print_P(html_open);
                char line1[170];
		sprintf(line1, "[\"%d\",\"%d\",\"%d\",\"%d\",\"%d\",\"%d\",\"%d\",\"%2.2d:%2.2d:%2.2d\"]", pit_temp, food_temp_0, food_temp_1, food_temp_2, food_temp_3,
				setpoint, fan_speed * 100 / 255, hours, minutes, seconds);
		WiServer.print(line1);
//		WiServer.print_P(html_close);
	...
	if (strncmp(URL, "/setpoint", 8) == 0) {
                setpoint = atoi(strncpy(URL, URL+19, 5));
//		sscanf(URL, "/setpoint?setpoint=%d", &setpoint);
		config.targetTemp = setpoint;
		writeConfig();
                char line1[15];
		sprintf(line1, "var setpoint=%d;s(setpoint);", setpoint);
		WiServer.print(line1);
		return true;
	}
...
void seconds_tick() {
	...
	food_temp_0 = thermister_temp(analogRead(3));
        food_temp_1 = thermister_temp(analogRead(2));
        food_temp_2 = thermister_temp(analogRead(1));
        food_temp_3 = thermister_temp(analogRead(0));
...
//        sprintf(line, "%2.2d:%2.2d:%2.2d Pit=%3d  Food=%3d  Setpoint=%d Blower=%d %c ", hours, minutes, seconds, pit_temp, food_temp, setpoint, fan_speed*100/255, wireless_stat);
	...
	lcd.clear();
	lcd.home();
        if(seconds_on_current  < 5) {
          if (current == 'P') {
            sprintf(line, "%2.2d:%2.2d %3d%c/%3d%c", hours, minutes, pit_temp, 0xDF, setpoint, 0xDF);
            lcd.print(line);
	    lcd.setCursor(0, 1);
            sprintf(line, "M0=%3d%c B=%2.2d%% %c", food_temp_0, 0xDF, fan_speed * 100 / 255, wireless_stat);       
            lcd.print(line);   
          } else {
            sprintf(line, "M0=%3d M1=%3d", food_temp_0, food_temp_1);                    
            lcd.print(line);
	    lcd.setCursor(0, 1);
            sprintf(line, "M2=%3d M3=%3d", food_temp_2, food_temp_3);                    	        
            lcd.print(line);
          }
          seconds_on_current++;
        } else  {
